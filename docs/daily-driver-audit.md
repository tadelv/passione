...
- **#5 accepted, local commit `1b058d2`:** home targets (including the header Scan) are at least 44×44px at the three audited sizes; configured brew temperature reuses the profile helper and an i18n label; last-shot legend wraps only in compact mode. Native sibling targets preserve keyboard access to profiles, coffee, recipe summary, shot detail, and Repeat without nested button semantics. Mobile center rows now respect content height rather than overlapping the chart and footer; narrow header telemetry no longer collides with the clock. Supervisor rejected the initial overlap and keyboard regressions, inspected corrected screenshots at 390×844, 768×1024 and 1024×768 plus mobile footer reachability, and independently passed build, 68/68 unit tests and 14/14 home/chart-navigation tests. Seven widget types, saved zones and layout format are unchanged.
- **#6 test isolation, local commit `de42cef`:** the previous #4 full-run 2-failure Repeat/History pair was an order dependency, not timing — the mock's shared `injectedShotIds` latest override from `post-shot-flow`'s `inject-fresh-shot` survived past the Repeat seed. Fixed with hermetic `beforeEach`/`afterEach` resets using the existing mock endpoints (never production shot semantics), real `expect.poll` waits replacing fixed sleeps, path-only machine-start detection, and a genuine mobile scrolled-footer assertion. Independent full nonscreenshot suite: **141/141 passed (9.2m)** with the previously failing pair green mid-run. Home screenshot assertion now verifies actual uPlot canvas boundaries and footer reachability.
