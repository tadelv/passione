...
const props = defineProps({
   ...
   * Expected shape (Streamline-Bridge format):
   * {
   *   elapsed: number[],
   *   pressure: number[],
   ...
   *   weight: number[],
   *   phaseMarkers?: { time: number, label: string, transitionReason?: string }[]
   * }
   */
...
function shotToData(shot) {
  if (!shot || !shot.elapsed?.length) return null

  return [
    shot.elapsed,
    shot.pressure ?? shot.elapsed.map(() => 0),
    shot.targetPressure ?? shot.elapsed.map(() => 0),
    shot.flow ?? shot.elapsed.map(() => 0),
    shot.targetFlow ?? shot.elapsed.map(() => 0),
    shot.temperature ?? shot.elapsed.map(() => 0),
    shot.targetTemperature ?? shot.elapsed.map(() => 0),
    shot.weight ?? shot.elapsed.map(() => 0),
  ]
}
...
function drawMarkers(u) {
  const markers = props.shot?.phaseMarkers
