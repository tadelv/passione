...
export function useRecipeOverlay(refs, ctx) {
  ...
  // Raises `refs.updating` across the whole (possibly slow, async bean/grinder
  // hydrated) load and keeps it raised through Vue's watcher flush (`await
  // nextTick()`) so the partial form never reaches the live-apply debounce.
  // Read-only callers follow this with overlayFromWorkflow(); a deliberate
  // user selection applies afterwards at the call site via applyToLiveWorkflow().
  async function loadFromPreset(index) {
    ...
    try {
      ...
      // Drain Vue's watcher flush while the guard is still raised.
      await nextTick()
    } finally {
  ...
  // ---- Mount hydration: restore the form from the live workflow ----
  // Gateway is authoritative for profile, context, AND operation flags/values,
  // regardless of whether a saved recipe is selected. Read-only: stays gated by
  // `refs.updating` through the async entity hydration and the watcher flush so
  // the overlay never publishes a write.
  async function overlayFromWorkflow() {
    if (!workflow) return
    refs.updating.value = true
    try {
      const ctxPayload = workflow.context ?? {}
      // Reconcile the entity links FIRST. The form may hold a stale/null link
      // (no saved-recipe baseline is loaded at mount), so align bean/grinder
      // to whatever the live workflow carries.
      ...
      // Skip coffeeName/coffeeRoaster while a bean is linked (bean record wins)
