...
- **`useProfilePresets`** — Static quick-start frame presets (Classic Espresso, Turbo Shot, Blooming Espresso, Allonge, Lever Style) for the profile editor's "New" flow. Exports `PROFILE_PRESETS`, an array of `{key, name, description, temperature, targetWeight, frames}` with frames precomputed at module load.
...
- **Workflow context merges are tri-state; explicit `null` clears.** `useWorkflow.applyData` (logic in `useWorkflowMerge.js`) treats a server-response own key as authoritative even when its value is `null` (an intentional clear), while a key the response omits keeps its current value (partial-update echo). Legacy top-level backfill (`doseData`/`grinderData`/`coffeeData`) only fills keys the context response did NOT explicitly carry, so an explicitly-cleared field is never resurrected. Builders for recipe/shot loading (`useComboApply.js`) always emit `beanBatchId`/`grinderId` — `null` when the source pins no link — because the workflow PUT deep-merges and an omitted ID silently keeps the previously-loaded association. Cross-reference: the bean-picker "Clear" button on `BeanPickerPopup`, and `useShotCache.refreshLatest` home flow, rely on explicit `null` clearing now that `??` is gone. (Audit #3 — note supervisor.)
...
- **Profiles are REA v2 `steps`, NOT de1app flat `frames`.** The gateway's `Profile.fromJson` (`vendor/reaprime/lib/src/models/data/profile.dart`) is strict: top-level array key is **`steps`** (`json['steps'] as List` — a non-null cast, so a missing/`frames` key 500s), each step needs `pump` (`pressure`|`flow`) + matching `pressure`/`flow`, `transition` (`fast`|`smooth`), `sensor` (`coffee`|`water`), and OPTIONAL nested `exit:{type,condition,value}` + `limiter:{value,range}`. Required top-level scalars `tank_temperature` and `target_volume_count_start` must be present — `parseDouble(null)`/`parseInt(null)` **throw**. The profile editor authors a de1app-derived flat frame internally (`exit_if`, `exit_pressure_over`, `max_flow_or_pressure`, …) — **all profile writes MUST go through `useProfileSerialize.buildReaProfile`** (and load through `reaProfileToInternal`). Never POST/PUT the editor's raw `profile.value`.
- **Profile saves use `useProfilePersist.persistProfile`, not raw `createProfile`/`updateProfile`.** Two REA contracts it enforces (see `vendor/reaprime/doc/Profiles.md`): (1) **Default (bundled) profiles can't PUT execution fields** — editing one must `POST` a child with `parentId` set to the default's id; `persistProfile` forks automatically when `record.isDefault`. (2) **A PUT that changes execution fields deletes the old record and returns a NEW content-hash id** — always adopt `result.id` into your local `record`, or the next save targets a deleted id (404). Also replace the route (`router.replace`) when the id changes, or the editor keeps pointing at the old record and reopening shows the original without the saved changes. Verified live: see `scratch/verify-serialize.mjs` (offline, 70 default profiles) + `scratch/live-test.mjs` (gateway round-trip).
- **Dirty-computed snapshots must be reactive.** The profile editor's `isDirty` computed compares `JSON.stringify(profile.value)` against a snapshot — keep the snapshot in a `ref` and read it as `originalSnapshot.value` inside the computed. A plain `let` snapshot is not tracked, so the computed keeps its stale cached result after save: the "Modified" badge stays lit, navigating away pops the unsaved-changes dialog, and Save & Leave fires a redundant second save.
- **Disabling steam needs `targetTemperature: 0`, not just `duration: 0`.** The gateway has two different "is steam enabled" checks that don't agree: the live `PUT /api/v1/workflow` handler derives it from `steamSettings.duration > 0` (`vendor/reaprime/lib/src/services/webserver/workflow_handler.dart`), but the boot/reconnect default-push re-applies the persisted workflow using `steamSettings.targetTemperature >= 130` (`vendor/reaprime/lib/src/controllers/de1_controller.defaults.dart`) — duration is irrelevant there. Sending `duration: 0` with `targetTemperature` left at a real value (e.g. 160) disables steam immediately but turns it back on at the next machine reconnect. Any code building a "steam disabled" workflow payload (`useComboApply.js`, `useRecipeLiveApply.js`) must zero both fields. Flush/hot-water have no equivalent temperature gate — `duration: 0` / `volume: 0` alone is fine there.

## Feature Scope

Core brewing flow, profile management (browse/search/favorites/frame-by-frame editor), shot history (list/detail/comparison/post-shot review/phase summary), auto-favorites, Visualizer import, layout customization, bean + grinder catalog (`/catalog` route with sub-tabs, reachable from the home-screen nav widget), bean info, screensaver (ambient glow / last shot recap / shot graph modes), power & sleep schedule management, descaling wizard, settings.
...
**Profile Editor:** Single frame-by-frame editor (`AdvancedProfileEditorPage.vue`, route `/advanced-editor/:id?`) — matches ReaPrime's native REA v2 `steps` schema directly, so every profile (including ones authored under the old Simple/Recipe split) opens the same way. `PROFILE_PRESETS` (`useProfilePresets.js`) seed a full frame set from one tap for common shot styles. Old `/simple-editor/:id?` and `/profile-editor/:id?` links redirect to `/advanced-editor/:id?`. Profiles' `legacy_profile_type`/`profile_type` tags from the old split (if present on older records) are inert — not read by anything.
...
- **AdvancedProfileEditorPage:** Frame-by-frame profile editor. Live graph preview. `PROFILE_PRESETS` quick-start pills seed a full frame set. Dirty detection via `onBeforeRouteLeave` catches every navigation away (bottom-nav, keyboard shortcuts, browser back), not just its own Back button. Routes via `ProfileInfoPage`'s Edit button and `ProfileSelectorPage`'s "+ New" button.
